...
function getSliderSentence(idx, val) {
  ...
  return val === 3
    ? `Your score indicates ${t}.`
    : `Your score indicates a ${t}.`;
}
...
function generatePDF(data) {
  return new Promise((resolve, reject) => {
    ...
    const CW = 500; // content width (612 - 56*2)
    const ML = 56;
    const ACCENT  = '#3C3489';
    const DARK    = '#1a1a2e';
    const MID     = '#5F5E5A';
    ...
    doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(22)
       .text('Culture Alignment Demo Assessment', ML, 28, { width: CW });
    ...
    const introLines = doc.font('Helvetica').fontSize(10).heightOfString(introText, { width: CW - 28, lineGap: 2 });
    const introH = introLines + 38;
    ...
    for (let p = 0; p < totalPages; p++) {
      ...
      doc.fillColor(LIGHT).font('Helvetica').fontSize(8)
         .text(`Culture Alignment Demo Assessment  ·  Confidential  ·  Page ${p + 1} of ${totalPages}`, ML, 756, { width: CW });
    }
...
module.exports = async (req, res) => {
  ...
  try {
    const pdfBase64 = await generatePDF(req.body);
    const filename = `culture-alignment-${(req.body.name || 'report').replace(/\s+/g, '-').toLowerCase()}.pdf`;
    res.status(200).json({ success: true, pdf: pdfBase64, filename });
  } catch (err) {
